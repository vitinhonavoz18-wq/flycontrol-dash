...
export function TablesManagement({ tenantId, restaurantSlug }: TablesManagementProps) {
  ...
  useEffect(() => {
    if (tables.length > 0 && !newTableNumber) {
      ...
      const numericTables = tables.map((t) => parseInt(t.table_number)).filter((n) => !isNaN(n));
      ...
      <html>
        ...
        <body>
          <div class="card">
            ...
            <div id="qrcode"></div>
            <div class="footer">FlyControl</div>
          </div>
  ...
  function handlePrintComanda(session: TableSession, orders: any[]) {
    ...
    const feeAmount = session.service_fee_enabled
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            disabled={tablesLoading || sessionsLoading}
          >
            <RefreshCw
              className={`h-4 w-4 mr-2 ${tablesLoading || sessionsLoading ? "animate-spin" : ""}`}
            />
            Atualizar
          </Button>
        </div>
        ...
        <TabsList className="grid w-full max-w-md grid-cols-2">
          ...
          <TabsTrigger value="sessions" className="gap-2">
            ...
            {sessions.length > 0 && (
              <Badge
                variant="destructive"
                className="ml-1 h-5 w-5 flex items-center justify-center p-0 rounded-full text-[10px]"
              >
                {sessions.length}
              </Badge>
            )}
        ...
        <TabsContent value="tables" className="space-y-6 pt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Adicione mesas individualmente para gerar seus QR Codes exclusivos.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3 items-end">
                ...
                <Button
                  ...
                  disabled={isAdding || !newTableNumber}
                  className="gap-2"
                >
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tables.map((table) => (
              <Card
                key={table.id}
                className={`overflow-hidden border-2 transition-all ${table.is_active ? "border-border" : "border-dashed opacity-60"}`}
              >
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        updateDefaultWaiter(table.id, val === "__none__" ? null : val)
                      }
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (
                          confirm(`Tem certeza que deseja excluir a Mesa ${table.table_number}?`)
                        ) {
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                        <Clock className="h-3 w-3" /> Aberta há{" "}
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="flex items-center justify-between gap-2 bg-background rounded-md border px-3 py-2">
                      <span className="text-sm font-medium truncate">
                        {session.waiter_name ?? (
                          <span className="text-muted-foreground">Sem garçom</span>
                        )}
                      </span>
                      <Button
                        size="sm"
                ...
                <CardContent className="p-5 space-y-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {new Intl.NumberFormat("pt-BR", {
                          style: "currency",
                          currency: "BRL",
                        }).format(session.subtotal_amount)}
                      </span>
                    </div>
                    {session.service_fee_enabled && (
                      <div className="flex justify-between text-orange-600">
                        ...
                        <span>
                          {new Intl.NumberFormat("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          }).format(session.service_fee_amount)}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      {session.service_fee_enabled
                        ? `Remover ${session.service_fee_percent}%`
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="col-span-2 gap-2 text-green-600 border-green-200 hover:bg-green-50"
                      onClick={() => {
                        setSelectedSession(session);
            ...
            {sessions.length === 0 && !sessionsLoading && (
              <div className="col-span-full py-20 text-center border-2 border-dashed rounded-xl text-muted-foreground">
                ...
                <p className="text-sm">
                  Os pedidos feitos via QR Code aparecerão aqui automaticamente.
                </p>
              </div>
            )}
          </div>
      ...
      {selectedSession &&
        ...
        (() => {
          ...
          const subtotal = validOrders.reduce((acc, o) => acc + (Number(o.total) || 0), 0);
          const feeAmount = selectedSession.service_fee_enabled
            ? subtotal * (selectedSession.service_fee_percent / 100)
            : 0;
          const total = subtotal + feeAmount;
                ...
                <CardContent className="flex-1 overflow-y-auto p-4 space-y-6">
                  {loadingOrders ? (
                    ...
                    <p className="text-center py-10 text-muted-foreground">
                      Nenhum pedido real vinculado a esta sessão.
                    </p>
                  ) : (
                              ...
                              orderItems.map((item: OrderItem, i: number) => (
                                ...
                                >
                                  <div className="flex justify-between text-sm">
                                    <span className="font-medium">
                                      {item.qty ?? item.quantity ?? 1}x {formatItemName(item)}
                                    </span>
                ...
                <div className="border-t p-4 bg-muted/20 space-y-3">
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button
                      variant="outline"
                      className="flex-1"
                      onClick={() => handlePrintComanda(selectedSession, sessionOrders)}
                    >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
              <Card className="w-full max-w-md">
                <CardHeader>
                  ...
                  <CardDescription>
                    Confira o resumo final antes de fechar a comanda.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4 py-4">
                  ...
                  {selectedSession.service_fee_enabled && (
                    <p className="text-[10px] text-center text-muted-foreground">
                      Taxa de serviço de {selectedSession.service_fee_percent}% aplicada ao total da
                      comanda.
                    </p>
                  )}
                </CardContent>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <RadioGroup
                value={changeWaiterMode}
                onValueChange={(v) => setChangeWaiterMode(v as "session" | "default")}
              >
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setChangeWaiterSession(null)}>
              Cancelar
            </Button>
            <Button
              onClick={async () => {
                ...
                await assignWaiter(changeWaiterSession.id, wid, {
                  alsoSetDefault: changeWaiterMode === "default",
                });
