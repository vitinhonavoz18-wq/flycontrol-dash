...
export const Route = createFileRoute("/api/sync-table-sessions")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        // Esta rota reescreve o valor das comandas usando a chave de serviço
        // (que ignora as regras de acesso do banco). Sem a conferência abaixo
        // ela aceitava o número de qualquer loja vindo de qualquer pessoa —
        // era como um caixa que refaz a conta de qualquer mesa de qualquer
        // restaurante para quem chegasse dizendo o número da mesa.
        let caller;
        try {
        ...
        try {
          ...
          if (!tenant_id) {
            return new Response(JSON.stringify({ success: false, error: "missing_tenant_id" }), {
              status: 400,
              headers: cors,
            });
          }

          // Dono da loja ou administrador da plataforma — mais ninguém.
          ...
          const { data: candidates, error: oErr } = await supabaseAdmin
            .from("orders")
            .select(
              "id, table_number, total, created_at, order_type, service_mode, delivery_type, table_id, status",
            )
          ...
          for (const sess of sessions || []) {
            ...
            const matching = (candidates || []).filter((o) => {
              ...
              if (
                sess.opened_at &&
                o.created_at &&
                new Date(o.created_at) < new Date(sess.opened_at)
              )
                return false;
              return true;
            });
            ...
            const { data: ordersInfo } = await supabaseAdmin
              ...
              .in(
                ...
                ).data?.map((r) => r.order_id) || ["00000000-0000-0000-0000-000000000000"],
              );

            const subtotal = (ordersInfo || [])
              .filter((o) => o.status !== "cancelado" && o.status !== "deleted")
              .reduce((sum, o) => sum + (Number(o.total) || 0), 0);
            const fee = sess.service_fee_enabled
              ? (subtotal * (Number(sess.service_fee_percent) || 10)) / 100
              : 0;
          ...
          // O detalhe do erro fica no log do servidor. Devolvê-lo ao cliente
          // entregaria nomes de tabelas e colunas de brinde.
